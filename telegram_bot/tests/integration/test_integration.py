...
# Предполагается, что функции импортируются из src.api_utils и src.database_utils
# Если структура вашего проекта немного отличается, скорректируйте импорты.
...
async def test_search_song_integration():
    ...
    mock_response.json.return_value = {
        "results": [
            {"id": "1", "title": "Test Song", "artists": ["Test Artist"], "duration": 180000, "url": "http://test"}
        ]
    }
...
async def test_search_song_empty_or_error():
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        mock_get.side_effect = httpx.HTTPStatusError("500 Server Error", request=MagicMock(), response=MagicMock())
...
async def test_trim_audio_boundary_exceeded():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
            mock_post.side_effect = httpx.HTTPStatusError("400 Bad Request", request=MagicMock(), response=MagicMock())
...
async def test_create_video_render_success():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(image_path, "wb") as f: f.write(b"image")
...
async def test_create_video_invalid_media():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(audio_path, "wb") as f: f.write(b"bad")
        with open(image_path, "wb") as f: f.write(b"bad")
